...
fn drift_core(m: &Bound<'_, PyModule>) -> PyResult<()> {
    ...
    m.add_function(wrap_pyfunction!(api::object_to_protobuf_struct_field_count, m)?)?;
    ...
    m.add_function(wrap_pyfunction!(api::build_export_spans_request_bytes_pyobject, m)?)?;
    Ok(())
}
