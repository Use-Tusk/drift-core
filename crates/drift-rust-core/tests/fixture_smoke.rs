...
fn basic_fixture_smoke() {
    ...
    assert!(
        !normalized.is_empty(),
        "normalized output should not be empty"
    );
    ...
    assert_eq!(
        normalized_via_combo, normalized,
        "combined API should match normalize_json"
    );
    assert_eq!(
        hash_via_combo, hash,
        "combined API should match deterministic_hash"
    );
    ...
    let fields =
        object_to_protobuf_struct_field_count(&input).expect("field count should be produced");
    assert!(
        fields > 0,
        "top-level protobuf struct should contain fields"
    );
}
